...
title: JavaScript e DOM do HTML
...
# Revisão de HTML: _Tags_ Típicas de Formatação
...
# Revisão de HTML: _Tags_ de Elementos Multimídia
...
# Revisão de HTML: Campos das _Tags_
...
# Revisão de HTML: Campos das _Tags_ (Exemplo)
...
# Revisão de HTML: Formulários (II)
...
# Revisão de HTML: Submetendo um Formulário
...
# Revisão de HTML: Criando um Simples Botão
...
- Linguagem de script para aplicações web.
	- Programação geralmente do lado do cliente.
- Código é escrito entre _tags_ `<script>` `</script>` no _head_ do HTML.
	- Ou através de um ficheiro .js que deve ser indicado também no _head_ do HTML.
- Linguagem de sintaxe parecida com Java e C.
- Veremos, a seguir, exemplos de uso do Javascript para manipular o DOM do HTML.
	- Isto é, manipular dinamicamente a estrutura do HTML.
- Caso queira se apronfundar na linguagem, aceder ao Tutorial do W3School.
...
1. Crie um ficheiro em HTML + JS que mostre imagens diferentes a cada 10 s.
	- Baseie-se no slide anterior para isso e inclua também botões para iniciar e parar.
	- No JS, crie um vetor ou objeto para armazenar os caminhos das imagens.
		- Um vetor é declarado da seguinte forma: `var vet = ["elem1", "elem2", "elem3"];`
		- Similar a lista de Python, mas declaração usando `var` e com ponto e vírgula ao final.
